...
class TestQUOTAEXCEEDED(unittest.TestCase):
    ...
    def make_instance(self, include_optional: bool) -> QUOTAEXCEEDED:
        if include_optional:
            return QUOTAEXCEEDED(errorMessage="Quota exceeded", errorCode="QUOTA_EXCEEDED")
        return QUOTAEXCEEDED()

    def test_quota_exceeded_minimal(self):
        inst = self.make_instance(include_optional=False)
        self.assertEqual(inst.to_dict(), {}, "Empty optional fields should not appear in dict output")

    def test_quota_exceeded_full(self):
        inst = self.make_instance(include_optional=True)
        data = inst.to_dict()
        self.assertEqual(data.get("errorMessage"), "Quota exceeded")
        self.assertEqual(data.get("errorCode"), "QUOTA_EXCEEDED")
