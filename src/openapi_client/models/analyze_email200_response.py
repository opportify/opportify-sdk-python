...
class AnalyzeEmail200Response(BaseModel):
    ...
    is_deliverable: StrictStr = Field(description="Checks if the email address exists and is deliverable using SMTP handshake simulation. This involves connecting to the mail server and issuing commands to verify deliverability. The possible answers…", alias="isDeliverable")
    ...
    email_correction: StrictStr = Field(description="The corrected email address when the system is highly confident about a typo or misspelling. Always present; empty string when no correction is needed.", alias="emailCorrection")
    email_auto_corrected_from: Optional[StrictStr] = Field(default=None, description="The original email address provided in the request, before auto-correction was applied. This field is only present when `enableAutoCorrection` is `true` AND the system automatically applied a…", alias="emailAutoCorrectedFrom")
